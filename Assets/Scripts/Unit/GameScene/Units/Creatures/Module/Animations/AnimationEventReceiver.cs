...
namespace Unit.GameScene.Units.Creatures.Module.Animations
{
    ...
    {
        public event Action OnActivateSkillEffect;
        ...
        public void SetFloat(AnimationParameterEnums para, float value, Action action)
        {
            _animator.SetFloat(_animationParameter[para], value, action);
        }
        
        public void SetTrigger(AnimationParameterEnums type, Action action)
        {
            _animator.SetTrigger(_animationParameter[type], action);
        }
        ...
        {
            if (OnActivateSkillEffect == null)
            {
                Debug.LogError("OnAttack이 비었습니다요");
            }
            
            OnActivateSkillEffect.Invoke();
        }
